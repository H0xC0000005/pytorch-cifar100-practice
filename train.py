...
def train(epoch, soft=True, epsilon=0.1):
    start = time.time()
    net.train()
    for batch_index, (images, labels) in enumerate(cifar100_training_loader):
        pass
        if len(labels.shape) == 1:
            # need to be one-hot to compare with mixup and softlabel, also softlabel must be one-hot
            labels = torch.eye(100)[labels]
        elif len(labels.shape) > 1:
            pass
        else:
            raise RuntimeError(f"shoudn't get label with dim<1 right?")
        if soft:
...
def eval_training(epoch=0, tb=True):
    ...
    if tb:
        writer.add_scalar(
            "Test/Average loss", test_loss / len(cifar100_test_loader.dataset), epoch
        )
        writer.add_scalar(
            "Test/Accuracy", correct.float() / len(cifar100_test_loader.dataset), epoch
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--batchsize", type=int, default=128, help="batch size for dataloader"
    )
    ...
    parser.add_argument(
        "--use_mixup", default=False, action="store_true", help="use mixup or not"
    )
    parser.add_argument("--use_softlabel", default=False, action="store_true")
    parser.add_argument("--use_epsilon", type=float, default=0.1)
    ...
    for epoch in range(1, settings.EPOCH + 1):
        ...
        train(epoch, soft=args.use_softlabel, epsilon=args.use_epsilon)
