...
from dataset import CIFAR100WithMixup
...
def get_training_dataloader(
    mean, std, batch_size=16, num_workers=2, shuffle=True, use_mixup=True
):
    ...
    # )
    print(f"> get use mixup arg: {use_mixup} with type {type(use_mixup)}")
    if use_mixup:
        cifar100_training = CIFAR100WithMixup(
            root="./data", train=True, download=True, transform=transform_train
        )
        print(f"get mixup=true")
    else:
        cifar100_training = torchvision.datasets.CIFAR100(
            root="./data", train=True, download=True, transform=transform_train
        )
        print(f"get mixup=false")
    print(f"> get dataset type {type(cifar100_training)}")
